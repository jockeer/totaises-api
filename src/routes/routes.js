...
router.get('/api/traerPartidosPorCampeonato/:id/:fecha', async (req, res, next) => {
    let all = await pool.query(`select pa.id, pa.fecha,pa.goles_local,pa.goles_visitante,el.nombre as "Equipo_Local",ev.nombre as "Equipo_Visitante",ar.nombre || ' ' || ar.apellido as "Arbitro",can.nombre as "Estadio",cam.nombre as "Campeonato"
